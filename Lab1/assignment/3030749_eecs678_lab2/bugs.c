...
struct Storage {
  ...
  char* sentence[7]; // Grow the array
...
void echoohce(char** strs) {
  ...
  for (iter = strs; *iter != NULL; ++iter)
    printf("%s ", *iter); // use *iter instead of iter
  ...
  // wrong.
  for (; iter != strs - 1; --iter) // fixed this because there is no null pointer
    printf("%s ", *iter);
...
int main(int argc, char** argv) {
  ...
  bug_info.sentence[6] = NULL; // add null value to stop echo
  ...
  free(bug_info.sentence[2]); // free memory first
