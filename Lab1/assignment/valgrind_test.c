...
int main() {
  ...
  for (uninitialized_variable = 0; uninitialized_variable < 100; uninitialized_variable++) { // give uninitiated_variable a value of 0
  ...
  free(*definitely_lost); // free the allocated memory
  free(definitely_lost); // free the allocated memory
  ...
  free(still_reachable); // free the allocated memory
  ...
  possibly_lost -=4; // reset the original position
  free(possibly_lost); // free the allocated memory
